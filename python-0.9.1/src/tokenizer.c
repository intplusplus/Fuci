...
struct tok_state *
tok_setupf(FILE *f, char *ps1,char *ps2)
{
       struct tok_state *tok = tok_new();
       if (tok == NULL)
               return NULL;
       ...
       char **p_start, **p_end; /* Out: point to start/end of token */
{
       ...
       /* 
          tok->atbol 不是0，说明在新行, tok->atbol == True
          获取当前行首的空格量，用来表示缩进的层级
       */
       if (tok->atbol) {
               register int col = 0;
               tok->atbol = 0; 
               ...
               /*当前c是第一个不是空格的字符，回退一下*/
               tok_backup(tok, c);
               /*当前空格和当前缩进层级的空格一样，说明缩进层级一样，没有变化*/
               if (col == tok->indstack[tok->indent]) {
                       /* No change */
               }
               
               /*tok->indent 表示当前层级*/
               /*tok->indstack 表示当前层级的空格数量*/
               /*缩进层级加一了*/
